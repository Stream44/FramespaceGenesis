...
import { registerRep, RenderItem, PrettyJson, vlog, splitFilePath, makeRelativePath } from "../../../../L13-workbench/vinxi-app/src/lib/renderLib";
import type { JsonObject, JsonValue } from "../../../../L13-workbench/vinxi-app/src/lib/renderLib";
...
registerRep({
    ...
    render: (data, ctx) => {
        ...
        // Title is $id relative to dirname of spineInstanceTreeId
        let title = id ?? capsuleName ?? hashType;
        let titleIsRelative = false;
        if (id && ctx.spineInstanceTreeId) {
            const baseDir = ctx.spineInstanceTreeId.substring(0, ctx.spineInstanceTreeId.lastIndexOf('/'));
            if (baseDir) {
        ...
        const handleLoad = async (e: MouseEvent) => {
            ...
            try {
                if (ctx.apiCall) {
                    ...
                    vlog("Capsule/Load", "No apiCall available in context");
                }
            } catch (err) {
                                    ...
                                    return (
                                        <Show when={hasTag} fallback={
                                            ...
                                            <RenderItem
                                                ...
                                                spineInstanceTreeId={ctx.spineInstanceTreeId}
                                            />
                                        </Show>
                                    );
                                }}
