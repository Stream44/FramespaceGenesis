...
const MOUNT_KEY = '@stream44.studio~FramespaceGenesis~L8-view-models~CapsuleSpine~Codepath~ModelQueryMethods'
...
const {
    ...
    const spine = await encapsulate({
        ...
        capsuleName: '@stream44.studio/FramespaceGenesis/L8-view-models/CapsuleSpine/Codepath/ModelQueryMethods.test',
    })
    return { spine }
...
for (const { spineInstanceTreeId } of trees) {
    ...
    describe(`Instance: ${spineInstanceTreeId}`, () => {
        ...
        it('getSwimlaneView', async () => {
            const result = await api.getSwimlaneView(spineInstanceTreeId)
